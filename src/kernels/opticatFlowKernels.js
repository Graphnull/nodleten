...
// }, 3 );
let glslProgramForCpu = ({ variableNames, outputShape, userCode }) => {
    return (...args) => {
        ...
        let passArgs = [out, Math.round, Math.abs, (v) => v, Math.floor];
        args.forEach((aa, j) => {
            let s = aa.shape;
            let t = aa.dataSync();
...
const opticalFlowFindKernel = (inputShape, backSize, kernelSize, startOffset, step) => {
    return {
...
const opticalFlowFindWebgl = (params) => {
    ...
    let backSize = [params.inputs.offset.shape[0], params.inputs.offset.shape[1]];
...
const opticalFlowFindCpu = (params) => {
    let kernelSize = params.attrs.kernelSize || 7;
    let startOffset = Math.floor(kernelSize / 2);
    let step = params.attrs.step || 1;
    let px = Math.floor((params.inputs.current.shape[2] - kernelSize - startOffset) / step);
...
module.exports.opticalFlowFind = ([offset, pred, current], { kernelSize = 7, step = 1 } = { kernelSize: 7, step: 1 }) => tf.engine().runKernel('opticalFlowFind', { offset, pred, current }, { kernelSize, step });
...
module.exports.blur = (a) => tf.engine().runKernel('blur', { a });
